...
#define UI_PURPOSE_RESETTIME   (140)                                        // 目的のUIを表示する時間
#define UI_PURPOSE_CUTALPAH    (-0.01f)                                     // 目的のUIの透明度を削る量
...
HRESULT CUI::Init(int nTimer, int nPlayerLife, float fPlayerOrbitSlash, float fPlayerSpecial, CTextureManager *pTextureManager)
{
	...
	// カウンターを初期化
	m_bPurposeDisp = true;
	m_nDispCounter = UI_PURPOSE_RESETTIME;
...
void CUI::Update(void)
{
	if (CManager::GetMode() == CManager::MODE_GAME)
	{// ゲーム中だったら
		if (m_bPurposeDisp == false)
		{// 目的を描画しない状態ならば
			D3DXCOLOR col;
			float fValue = UI_PURPOSE_CUTALPAH;
			// 現在のステージ数
			if (m_pNumStage != NULL)
			{// 生成されている
...
//    目的のUIを再表示する処理
//=============================================================================
void CUI::ResetPurpose(void)
{
	// 現在のステージ数
	if (m_pNumStage != NULL)
	{// 生成されている
		m_pNumStage->SetCol(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
	}

	// 目的
	if (m_pPurpose != NULL)
	{// 生成されている
		m_pPurpose->SetCol(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
	}

	// 紙背景
	if (m_pPaper != NULL)
	{// 生成されている
		m_pPaper->SetCol(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
	}
...
void CUI::DrawUI_Purpose(void)
{
	...
	if (m_pPaper != NULL)
	{// 生成されている
		ColDef = m_pPaper->GetCol();
		m_pPaper->SetCol(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
		m_pPaper->Draw();
		m_pPaper->SetCol(ColDef);
	}

	// 現在のステージ数
	if (m_pNumStage != NULL)
	{// 生成されている
		ColDef = m_pNumStage->GetCol();
		m_pNumStage->SetCol(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
		m_pNumStage->Draw();
		m_pNumStage->SetCol(ColDef);
	}

	// 目的
	if (m_pPurpose != NULL)
	{// 生成されている
		ColDef = m_pPurpose->GetCol();
		m_pPurpose->SetCol(D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
		m_pPurpose->Draw();
		m_pPurpose->SetCol(ColDef);
	}
}
...
//=============================================================================
//    目的の背景ポリゴンクラスへのポインタを取得する処理
//=============================================================================
CScene2D *CUI::GetPaper(void)
{
	return m_pPaper;
}